...
3. `書式プロファイル` シートができ、以降に生成するシフト表がこの書式になります
...
プロファイルは**スプレッドシート上のシート**に置き、リポジトリには入れません。
値は手で直せます。空欄にすればその項目だけ既定値に戻ります。
知らないキーや壊れた値は捨てて既定値に落ちるので、触っても壊れません。
...
| シート | 役割 | 作り方 |
|---|---|---|
| `書式プロファイル` | 生成時に使う書式の寸法 | 初期設定 → 実物の書式を取り込む |
